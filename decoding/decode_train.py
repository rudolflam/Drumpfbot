...
import decode_homogeneous_data

from theano.sandbox.rng_mrg import MRG_RandomStreams as RandomStreams
from collections import defaultdict

from decode_utils import *
from decode_layers import get_layer, param_init_fflayer, fflayer, param_init_gru, gru_layer
from decode_optim import adam
from decode_model import init_params, build_model, build_sampler
from decode_vocab import load_dictionary
from decode_search import gen_sample

# main trainer
def trainer(X, C, stmodel,
            dimctx=300, #vector dimensionality
            ...
            optimizer='adam',
            batch_size = 16,
            saveto='decode_model.npz',
            dictionary='../social_media.pkl',
            embeddings=None,
            saveFreq=10,
            sampleFreq=100,
            reload_=False):
    ...
    train_iter = decode_homogeneous_data.HomogeneousData([X,C], batch_size=batch_size, maxlen=maxlen_w)
    ...
    for eidx in xrange(max_epochs):
        ...
        for x, c in train_iter:
            n_samples += len(x)
            uidx += 1
            print 'Preparing data for epoch ', uidx
            x, mask, ctx = decode_homogeneous_data.prepare_data(x, c, worddict, stmodel, maxlen=maxlen_w, n_words=n_words)
            print 'Finished preparing data for epoch ', uidx
            if x == None:

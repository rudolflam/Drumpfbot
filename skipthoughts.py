...
def encode(model, X, d, use_norm=False, verbose=True, batch_size=128, use_eos=False):
    """
    ...
    print 'Initializing the features matrix'
    ufeatures = numpy.zeros((len(X), model['options']['dim']), dtype='float32')
    ...
    for k in ds.keys():
        ...
        for minibatch in range(numbatches):
            ...
            if use_eos:
                uembedding = numpy.zeros((k+1, len(caps), model['options']['dim_word']), dtype='float32')
                #bembedding = numpy.zeros((k+1, len(caps), model['options']['dim_word']), dtype='float32')
            else:
                uembedding = numpy.zeros((k, len(caps), model['options']['dim_word']), dtype='float32')
                #bembedding = numpy.zeros((k, len(caps), model['options']['dim_word']), dtype='float32')
            for ind, c in enumerate(caps):
                caption = captions[c]
                for j in range(len(caption)):
                    if caption[j] in d:
                        uembedding[j,ind] = model['table'][caption[j]]
                        #bembedding[j,ind] = model['table'][caption[j]]
                    else:
                        uembedding[j,ind] = model['table']['UNK']
                        #bembedding[j,ind] = model['table']['UNK']
                if use_eos:
                    uembedding[-1,ind] = model['table']['<eos>']
                    #bembedding[-1,ind] = model['table']['<eos>']
            if use_eos:
